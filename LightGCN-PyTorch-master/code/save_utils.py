...
def save_single(valid, test, market, save_path='result/temp', remove_oral_file=True):
    '''
     submission.zip
        ├── t1
        ...
        ├── t2
            ...
            └── valid_pred.tsv    /* scores of validation items */
    '''
    t_save_path = os.path.join(save_path, market)
    if not os.path.exists(t_save_path):
        os.makedirs(t_save_path)

    valid = valid[['userId', 'itemId', 'score']]
    test = test[['userId', 'itemId', 'score']]

    valid.to_csv(os.path.join(t_save_path, 'valid_pred.tsv'), sep='\t', index=None)
    test.to_csv(os.path.join(t_save_path, 'test_pred.tsv'), sep='\t', index=None)
