...
def read_run_file(run_file, score_columns='score'):
    qret = {}
...
def offline_scores(df, score_columns, gt_path, market_name):
    '''
    df: valid_df.
    score_columns: columns need to evaluation.
    gt_path: path of groudtruth.
    print:
        nDCG@10 and recall@10
    '''
    new_df = df.copy()
    new_df['score'] = df[score_columns]
    task_ov_val = get_df_scores_for_market(new_df, gt_path, market_name)
    return task_ov_val


def parse_predict_df(df_qret):
    qret = {}
    for row in df_qret.itertuples():
        cur_user_qret = qret.get(str(row.userId), {})
        cur_user_qret[str(row.itemId)] = float(row.score)
...
def get_df_scores_for_market(predict_df, data_dir, market_name):
    # prepare for val set
    ref_path_val = os.path.join(data_dir, 'valid_qrel.tsv')

    my_valid_run = parse_predict_df(predict_df)
    my_valid_qrel = read_qrel_file(ref_path_val)
    task_ov_val, task_ind_val = get_evaluations_final(my_valid_run, my_valid_qrel)
...
# python validate_submission.py --submission_file ../result/tmptime_12162307/submission.zip --data_dir ../../input
