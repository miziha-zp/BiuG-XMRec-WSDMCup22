...
def score_for_series(sim, num, item_users, data, topK=10, prefix=''):
    ...
    # data[prefix+'_swingi2iscore'] = cnt_list
    data[prefix+'__swingi2iscore_weighted'] = weighted_cnt_list
    return data
# 
def Swing_i2i_score(train_data, lgb_valid_data, lgb_test_data, K=10, prefix='train5core'):
    ...
    num = {}
    print('计算相似度...')
    sim = SwingRecall(trn_user_items)
    # 计算item相似度矩阵
...
def get_swingi2i_score(train, train5core, lgb_valid_data, lgb_test_data, K_list=[100]):
    ...
    for K in K_list:
        print('-'*20, K, '-'*20)
        # lgb_valid_data, lgb_test_data = Swing_i2i_score(train5core, lgb_valid_data, lgb_test_data,K, prefix='train5core@K={}'.format(K))
        # lgb_valid_data, lgb_test_data = Swing_i2i_score(train, lgb_valid_data, lgb_test_data, K, prefix='train@K={}'.format(K))
        train_data = merge_df(train5core, train, 0)
        lgb_valid_data, lgb_test_data = Swing_i2i_score(train_data, lgb_valid_data, lgb_test_data, K, prefix='concat_train@K={}'.format(K))
