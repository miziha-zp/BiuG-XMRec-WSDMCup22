...
def get_global_item_count():
    ...
    global_count_ = s1train5core.groupby('itemId',as_index=False)['userId'].count().rename(columns={"userId":"s1train5core_count"})
...
def get_feature_here(lgb_valid_data, lgb_test_data):
    def here_fun(df):
        ...
        for col in score_list:
            ...
            df['itemId_{}_mean'.format(col)] = df_view
...
def get_w2v_features(train, train5core, lgb_valid_data, lgb_test_data, emb_sz=8):
    ...
    for f1, f2 in emb_cols:
        emb1 = emb(train5core, f1, f2, emb_sz)
        emb2 = emb(train5core, f2, f1, emb_sz)
